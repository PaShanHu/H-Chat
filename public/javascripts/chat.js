$(document).ready(function() {
	...
	socket.on('say', function (data) {
		//对所有人说
		if (data.to == 'all') {
			$("#contents").append('<div>' + data.from + '(' + now() + ')对 所有人 说：<br/>' + data.message + '</div><br />');
		}
		//对你密语
		if (data.to == from) {
			$("#contents").append('<div style="color:#00f" >' + data.from + '(' + now() + ')对 你 说：<br/>' + data.message + '</div><br />');
		}
	});
	...
	$('#say').click(function(){
		...
		// 发送发话信息
		socket.emit('say', {from: from, to: to, message: msg});
		//清空输入框并获得焦点
